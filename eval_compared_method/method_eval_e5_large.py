# eval_e5_large.py
# Zero-shot eval for intfloat/multilingual-e5-large on ChunQiu benchmark (same eval combos)
#
# Install:
#   pip install -U transformers torch
#
# Run:
#   CUDA_VISIBLE_DEVICES=0 python method_eval_e5_large.py ./dataset
# or:
#   CUDA_VISIBLE_DEVICES=0 python method_eval_e5_large.py --data_dir ./dataset
#
# Notes (official):
#   - Each input text should start with "query: " or "passage: " (even for non-English).
#   - Use average_pool (masked mean) on last_hidden_state then L2 normalize.
...
DEFAULT_MODEL_NAME = "/amax/wangyh/pretrained/multilingual-e5-large"  # or local path
QUERY_PREFIX = "query: "
...
def average_pool(last_hidden_states: Tensor, attention_mask: Tensor) -> Tensor:
    # official snippet: masked mean pooling
...
def encode_e5_texts(
    ...
    max_length = min(int(max_length), 512)  # model card: long texts truncated to <=512 tokens
...
def encode_queries_e5(queries, tokenizer, model, device, batch_size, max_length, use_fp16) -> torch.Tensor:
    # retrieval task: query gets "query: " prefix
    texts = [QUERY_PREFIX + q for q in queries]
    return encode_e5_texts(texts, tokenizer, model, device, batch_size, max_length, use_fp16)


def encode_corpus_e5(texts, tokenizer, model, device, batch_size, max_length, use_fp16) -> torch.Tensor:
    # retrieval task: passage gets "passage: " prefix
...
def main():
    ...
    print("[EVAL] prefixes: query='query: ' passage='passage: '")
    ...
    if log_path:
        with open(log_path, "a", encoding="utf-8") as f:
            ...
            f.write("prefix_query='query: ' prefix_passage='passage: '\n")
    ...
    full_gallery_embs = encode_corpus_e5(
    ...
    for split in ("val", "test"):
        ...
        query_embs_full = encode_queries_e5(
